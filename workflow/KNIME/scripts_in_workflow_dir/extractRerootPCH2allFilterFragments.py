...
def extractInfoFromRerootedTrees(treeextracts, treepath):
	# initialize class
...
@click.command()
@click.option('--openmsassaylib', '-in', multiple = False, type = click.Path(), help = 'Input assay library from OpenMS::AssayGeneratorMetabo (.tsv,.traML,.pqp)')
@click.option('--rerootedtrees', '-trees', multiple = False, type = click.Path(), help = 'Input directory with rerooted trees from passatuto.')
@click.option('--decoyassaylib', '-out', multiple = False, type = click.Path(), help = 'Output of the decoy assay library (.tsv, .pqp)')
@click.option('--addms1shift', '-s', is_flag=True, help = 'Add CH2 (14.0156500641u) to the deocy precursor (not recommended)')

def main(openmsassaylib, rerootedtrees, decoyassaylib, addms1shift):
	...
	for entry in os.listdir(rerootedtrees):
		if entry.endswith(".txt"):
			...
			extractInfoFromRerootedTrees(treeextracts, treepath)
	...
	for key, value in d_lib_decoy.items():
		...
		for entry in unique_extracts:
			if str(entry.compoundname) == db_name and str(entry.adduct) == db_adduct and db_name.find("_decoy") == -1:
				value['TransitionGroupId'] = value['TransitionGroupId'] + '_decoy'
				value['TransitionId'] = value['TransitionId'] + '_decoy'
				value['CompoundName'] = value['CompoundName'] + '_decoy'
				value['Decoy'] =  1
				value['Annotation'] = 'NA'
